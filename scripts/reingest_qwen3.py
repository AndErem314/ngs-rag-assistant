#!/usr/bin/env python3
"""Re-ingest ChromaDB with qwen3-embedding:0.6b embeddings."""
...
print("\nStep 2: Re-embedding with qwen3-embedding:0.6b...")
...
for i in range(0, total, BATCH_SIZE):
    ...
    try:
        response = ollama_client.embed(model=QWEN3_MODEL, input=batch)
...
print(f"  Re-added {len(ids)} chunks with qwen3 embeddings")

verify = col.get(include=["documents", "metadatas"])
print(f"\nStep 4: Verification - {len(verify['ids'])} chunks in collection")

# Test query
test_embed = ollama_client.embed(model=QWEN3_MODEL, input="Nextera XT bead ratio for small fragments")
